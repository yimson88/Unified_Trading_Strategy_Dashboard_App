...
st.set_page_config(
    page_title="Unified Trading Strategy Dashboard",
    page_icon="📊",
    layout="wide"
)
...
def price_fmt(market):
    return f"%.{PAIRS[market]['decimals']}f"
...
def rr_value(label):
    return RR_OPTIONS[label]

def stop_presets(market):
    if market == "BTCUSD":
        return {"Tight $500": 500.0, "Normal $1000": 1000.0, "Wide $1500": 1500.0, "Very Wide $2000": 2000.0}
    if market == "XAUUSD":
        return {"Tight $5": 5.0, "Normal $10": 10.0, "Wide $15": 15.0, "Very Wide $20": 20.0}
    if "JPY" in market:
        return {"Tight 15 pips": 0.15, "Normal 25 pips": 0.25, "Wide 40 pips": 0.40, "Very Wide 60 pips": 0.60}
...
def pip_value_standard_lot(market, entry, usd_jpy, aud_usd):
    ...
    if market in ["XAUUSD", "BTCUSD"]:
        return None

    if market.endswith("USD"):
        return contract * pip

    if market.startswith("USD"):
        return (contract * pip) / entry

    if "JPY" in market:
        return (contract * pip) / usd_jpy

    if market == "EURAUD":
        return (contract * pip) * aud_usd
...
def calc_position(market, direction, entry, sl, tp, balance, risk_percent, usd_jpy, aud_usd):
    ...
    if direction == "BUY" and not (sl < entry < tp):
        raise ValueError("BUY trade needs SL below entry and TP above entry.")

    if direction == "SELL" and not (tp < entry < sl):
        raise ValueError("SELL trade needs TP below entry and SL above entry.")
...
def backtest_signals(m15, market, balance, risk_percent, max_hold):
    ...
    while i < len(d) - 2:
        ...
        if exit_reason == "TIME OUT":
            result = "Timeout Positive" if pnl >= 0 else "Timeout Negative"

        trades.append({
            "Signal Date": row["Date"],
            "Signal Cameroon Time": row.get("Cameroon_Time", "N/A"),
            "Direction": direction,
            "Entry": round(entry, PAIRS[market]["decimals"]),
            "SL": round(sl, PAIRS[market]["decimals"]),
            "TP": round(tp, PAIRS[market]["decimals"]),
            "Exit": round(exit_price, PAIRS[market]["decimals"]),
    ...
    equity_df = pd.DataFrame({"Trade": range(len(eq)), "Balance": eq})
...
def color_rows(df):
    def apply(row):
        ...
        if res == "WIN":
            return [f"background-color: {WIN_BG}; color: #065f46"] * len(row)
        if res == "LOSS":
            return [f"background-color: {LOSS_BG}; color: #991b1b"] * len(row)
        if "TIME" in exit_reason or "TIMEOUT" in res:
            if pd.notna(pnl) and pnl >= 0:
                return [f"background-color: {TIME_POS_BG}; color: #166534"] * len(row)
            if pd.notna(pnl) and pnl < 0:
                return [f"background-color: {TIME_NEG_BG}; color: #991b1b"] * len(row)
        if "BUY" in sig:
            return [f"background-color: {BUY_BG}; color: #065f46"] * len(row)
        if "SELL" in sig:
            return [f"background-color: {SELL_BG}; color: #991b1b"] * len(row)
        if "NEUTRAL" in sig or sig == "":
...
st.markdown('<div class="main-title">📊 Unified Trading Strategy Dashboard</div>', unsafe_allow_html=True)
st.markdown('<div class="subtitle">Switch strategies with toggle buttons, analyze signals, calculate risk, and record trades in one clean journal.</div>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    market = st.selectbox("Market", list(PAIRS.keys()), index=0)
    ...
    try:
        ...
        if analysis_error is None:
            if strategy == "SMC Market Structure":
                daily, h1, m15 = build_smc_strategy(
                    daily, h1, raw15, rr_ratio, atr_mult, swing_len, strict_mode,
                    session_start, session_end, enforce_session
                )
            else:
                daily, h1, m15 = build_fx15_strategy(
                    daily, h1, raw15, rr_ratio, atr_mult,
                    session_start, session_end, enforce_session
                )
            ...
            if run_backtest:
                trades_bt, equity_bt = backtest_signals(m15, market, account_balance, risk_percent, max_hold)
...
if st.session_state["page"] == "Dashboard":
    ...
    live_price, source = get_live_price(market)

    k1, k2, k3, k4, k5 = st.columns(5)
    k1.metric("Active Strategy", strategy)
    k2.metric("Market", market)
    k3.metric("Live Price", fmt_price(market, live_price))
    k4.metric("Risk", f"{risk_percent}%")
    k5.metric("R:R", rr_label)
    st.caption(f"Live price source: {source}. Yahoo prices may be delayed; confirm with your broker.")

    if analysis_error:
        st.error(analysis_error)
    elif strategy != "Manual Risk Calculator" and latest is not None:
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Latest Signal", latest.get("Signal", "NEUTRAL"))
        c2.metric("Direction", latest.get("Direction", "NEUTRAL"))
        c3.metric("Cameroon Time", latest.get("Cameroon_Time", "N/A"))
        c4.metric("Trading Window", "YES" if bool(latest.get("Trading_Window", False)) else "NO")

        show_cols = [c for c in ["Date", "Cameroon_Time", "Trading_Window", "Strategy", "Signal", "Direction", "Daily_Bias", "H1_Structure", "Premium_Discount", "Entry", "SL", "TP", "Reason"] if c in m15.columns]
        st.subheader("Latest Market Rows")
        st.dataframe(color_rows(m15.tail(30)[show_cols]), use_container_width=True)

    st.subheader("Journal Snapshot")
    if journal.empty:
        st.info("No trades recorded yet.")
    else:
        st.dataframe(color_rows(journal.sort_values("Trade_ID", ascending=False).head(20)), use_container_width=True)
...
# =====================================================
    ...
    if strategy == "Manual Risk Calculator":
        st.info("Manual Risk Calculator has no strategy signal. Go to Trade Ticket to plan and save a manual trade.")
    elif analysis_error:
        st.error(analysis_error)
    else:
        ...
        if setups.empty:
            st.warning("No valid setups found in the available data.")
        else:
            ...
            selected_index = st.selectbox("Choose setup to send to Trade Ticket", list(setups.index), index=len(setups.index) - 1)
            if st.button("Send Selected Setup to Trade Ticket"):
                send_to_ticket(strategy, market, setups.loc[selected_index], risk_percent, rr_label)

        if run_backtest:
            st.subheader("Backtest Trades")
            if trades_bt.empty:
                st.info("No backtest trades found.")
            else:
                st.dataframe(color_rows(trades_bt.tail(100)), use_container_width=True)
                st.download_button("Download Backtest CSV", trades_bt.to_csv(index=False).encode("utf-8"), f"{market}_{strategy}_backtest.csv", "text/csv")
...
# =====================================================
    ...
    st.subheader("Trade Ticket / Risk Calculator / Record Trade")
    ...
    entry_default = ticket.get("Entry", live_price)
    if pd.isna(entry_default):
        entry_default = live_price
    ...
    step = price_step(trade_market)
    fmt = price_fmt(trade_market)
    ...
    stop_distance = presets[preset_name]

    auto_sl, auto_tp = sl_tp_from_distance(direction, entry, stop_distance, rr_ticket)

    use_strategy_sl_tp = False
    if pd.notna(sl_default) and pd.notna(tp_default):
        use_strategy_sl_tp = r4.checkbox("Use strategy SL/TP", value=True)
    else:
        r4.caption("Using profile SL/TP")

    if use_strategy_sl_tp:
        sl = float(sl_default)
        tp = float(tp_default)
    else:
        sl, tp = auto_sl, auto_tp
    ...
    try:
        ...
        if risk_pct > 2:
            st.error("High risk: above 2% of account.")
        elif risk_pct > 1:
            st.warning("Caution: above 1% of account.")
        else:
            st.success("Risk is within controlled range.")

    except Exception as e:
        pos = None
        st.error(str(e))

    st.caption(f"Live default source: {source}. Always confirm the final price in your broker platform.")
    ...
    mistake = j7.selectbox("Mistake", ["None", "Entered too early", "Entered too late", "Moved SL", "Over-risked", "Ignored trend", "Ignored news", "Revenge trade", "Did not follow plan", "Other"])

    notes = st.text_area("Notes / Lesson learned", value=ticket.get("Reason", ""))
    ...
    if st.button("Save Trade", type="primary", disabled=(pos is None)):
        ...
        new_row = {
            "Trade_ID": next_trade_id(journal),
            "Date": trade_date,
            "Strategy": trade_strategy,
            "Market": trade_market,
            "Direction": direction,
            "Signal": ticket.get("Signal", direction),
            "Session": session,
            "Entry": entry,
            "Stop_Loss": sl,
            "Take_Profit": tp,
            "Risk_Percent": risk_pct,
            "Risk_Amount": pos["risk_amount"] if pos else np.nan,
            "Lot_Size": pos["lot_size"] if pos else np.nan,
            "Position_Size": pos["position_size"] if pos else np.nan,
            "Risk_Reward": rr_ticket_label,
            "Result": result,
            "Exit_Price": np.nan if exit_price == 0 else exit_price,
            "Exit_Reason": exit_reason,
            "PnL": pnl,
            "R_Multiple": r_mult,
            "Emotion": emotion,
...
# =====================================================
    ...
    if journal.empty:
        ...
        edit_step = price_step(edit_market)
        edit_fmt = price_fmt(edit_market)
        ...
        if st.button("Update Trade", type="primary"):
            ...
            journal.loc[idx, "PnL"] = pnl
            journal.loc[idx, "R_Multiple"] = r_mult
            journal.loc[idx, "Notes"] = edit_notes
            save_journal(journal)
    ...
    if journal.empty:
        ...
        for _, r in closed.sort_values("Trade_ID").iterrows():
            if pd.notna(r["PnL"]):
                bal += r["PnL"]
            equity.append(bal)
            trade_axis.append(r["Trade_ID"])

        eq = pd.DataFrame({"Trade": trade_axis, "Balance": equity})
        st.plotly_chart(equity_chart(eq), use_container_width=True)
        ...
        st.dataframe(color_rows(by_strategy), use_container_width=True)

        st.subheader("Performance by Market")
        by_market = closed.groupby("Market").agg(
            Trades=("Trade_ID", "count"),
            Net_PnL=("PnL", "sum"),
            Net_R=("R_Multiple", "sum"),
            Avg_R=("R_Multiple", "mean")
        ).reset_index()
        st.dataframe(color_rows(by_market), use_container_width=True)
